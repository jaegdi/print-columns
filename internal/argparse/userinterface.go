...
/**
 * @brief Collects unknown parameters as column numbers.
 *
 * Parses command-line arguments to identify integer values or ranges (m:n) representing column numbers.  Handles both ascending and descending ranges.  Returns an error if the input format is invalid.
 * @return T_ColNumbers A slice of column numbers.
 */
...
/**
 * @brief Disables CmdParams that are incompatible with CSV or JSON output.
 *
 * If CmdParams.Csv or CmdParams.Json is true, disables certain CmdParams that are not meaningful in CSV or JSON formats.
 */
...
/**
 * @brief Evaluates command-line flags and sets the CmdParams struct.
 *
 * Parses command-line flags using the flag package, sets the values to the CmdParams struct, and performs additional parameter validation and adjustments.  Prints help or manual if requested.
 */
