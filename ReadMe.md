...
DESCRIPTION
    ...
    Typical usage is to format, rearrange, filter or reorder the tabular output of commands.

    For example, when you got output from a command in a shell loop, then it is typical that the output on every loop
    has different column width. In this case pipe the output of the complete loop to pc,
    eg.
        for e in $(get items); do
            cmd with params
        done | pc
    ...
    The parameters:
        ...
        -sep=' '                           define the seperator, which is used to split the input data, default is blank ' '
        ...
        -colsep='|'       ColumnSeparator  define the character to separate the output columns, default='|'.
                                           To seperate output columns by 2 spaces instead of one, use: -colsep=' ' -cs
        ...
        -ts               TitleSeparator,  draws a separator line between first(header) and second line of output.
        ...
        1 2 m n      ColumnNumbers of input columns to output,
...
AUTHOR
    ...
    written by Dirk Jäger (dirk.jaeger.dj@gmail.com) as an exercise to learn golang.
...
Usage: pc [OPTIONS] [column-numbers] ...
...
EXAMPLES
    ...
    - Format the output of a command
        cmd: oc get pod -o wide --all-namespaces |head -n15| pc -ts -cs  8 1 2 5 6
            prints input col 1 2 5 6 8 in the output order 8 1 2 5 6 with title seperator and column seperator
        result:
